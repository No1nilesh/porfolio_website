...
function AboutCard({ index, details }) {
  ...
  return (
    <div
      id={"info" + index}
      className="gradient-box custom_shadow basis-1/3 w-full rounded-xl drop-shadow-xl info"
    >
      <div className="about-card h-full  w-full p-6 bg-background rounded-xl  flex flex-col justify-center items-center ">
        <FontAwesomeIcon
          icon={iconMap[details.icon]}
          style={{ color: "hsl(207, 90%, 72%)", height: "26px" }}
        />
        <h1 className="font-bold text-base">{details.title}</h1>
        <p className="text-small_text text-xs">{details.paragraphs[0]}</p>
        <p className="text-small_text text-xs">{details.paragraphs[1]}</p>
      </div>
    </div>
  );
}
