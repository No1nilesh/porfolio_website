...
export default function DeveloperCore({
  autoCycleInterval = 4500,
}: DeveloperCoreProps) {
  const [activeIdx, setActiveIdx] = useState(0);
  const [isPaused, setIsPaused] = useState(false);
  const [pulseKey, setPulseKey] = useState(0);
  const [omnitrixMode, setOmnitrixMode] = useState(false);
  const { setGalvanic } = useGalvanic();

  const enterOmnitrix = () => {
    setOmnitrixMode(true);
    setGalvanic(true);
  };

  const exitOmnitrix = () => {
    setOmnitrixMode(false);
    setGalvanic(false);
  };
    ...
    <div className="relative flex flex-col items-center w-full h-full min-h-105 py-2 select-none">
      <AnimatePresence mode="wait" initial={false}>
        ...
        {omnitrixMode ? (
          ...
          >
            <OmnitrixCore onExit={exitOmnitrix} />
          </motion.div>
        ) : (
          /* ── ARC REACTOR MODE (your existing code, untouched) ──────────── */
          <motion.div
            key="rc"
        ...
        ================================================================ */}
            ...
            <div className="text-center mt-1 px-2 max-w-sm">
              ...
              {/* ── OMNITRIX MODE TOGGLE ──────────────────────────────────────── */}
              <div className="mt-2.5">
                <button
                  onClick={enterOmnitrix}
